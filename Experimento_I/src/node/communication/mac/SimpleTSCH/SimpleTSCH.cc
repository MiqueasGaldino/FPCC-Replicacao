...
void SimpleTSCH::startup() {
	...
	for(int i = 0; i < this->superFrameSize; i++) {
		...
		k = k+2;
		trace() << "DIEGO this->slots[i]=" << this->slots[i];
	}
...
void SimpleTSCH::fromNetworkLayer(cPacket * pkt, int destination)
{
	...
	p->pkt = pkt;
	p->dest = destination;
	trace() << "DIEGO fromNetworkLayer destination=" << destination;
	p->ack = true;
	buffer.push_back(p);
}
...
void SimpleTSCH::fromNetworkLayer(cPacket * pkt, int destination, bool ack)
{
	...
	p->pkt = pkt;
	p->dest = destination;
	trace() << "DIEGO fromNetworkLayer destination=" << destination << " book ack=" << ack;
	p->ack = ack;
	buffer.push_back(p);
}
...
void SimpleTSCH::fromRadioLayer(cPacket * pkt, double rssi, double lqi)
{
	...
	if(macPkt->getKindTSCH() == ACK_PACKET && macPkt->getDestination() == SELF_MAC_ADDRESS) {
		...
		trace() << "DIEGO Received ack for packet " << macPkt->getSequenceNumber() << " fromRadioLayer";
		if(buffer_ret.size() > 0 && macPkt->getSequenceNumber() == buffer_ret.front()->packet->getSequenceNumber()) {
			...
			trace() << "DIEGO Linha 79 Retirando pacote para retransmissão...";
			//delete pr;
		}
		else if(buffer_ret.size() > 0 && macPkt->getSequenceNumber() > buffer_ret.front()->packet->getSequenceNumber()) {
			trace() << "DIEGO chegou ack da frente...";
			while(buffer_ret.size() > 0 && macPkt->getSequenceNumber() > buffer_ret.front()->packet->getSequenceNumber()) {
				trace() << "DIEGO Linha 85 Retirando pacote para retransmissão... " << buffer_ret.front()->packet->getSequenceNumber();
				buffer_ret.pop_front();
			}
		}
	}
	...
	{		
		...
		trace() << "DIEGO Enviado para camada de rede o pacote ACK, espera 2ms pra transmitir";
		//send an ACK_PACKET
...
void SimpleTSCH::timerFiredCallback(int index)
{
	...
	switch (index){
		...
		case SEND_ACK: {
			trace() << "DIEGO Sending ack for packet: " << macFrameAck->getSequenceNumber();
		...
		case SWITCH_CHANNEL: { 
			...
			trace() << "DIEGO ASN=" << ASN << " superFrameSize=" << superFrameSize << " indch=" << indch;
			
			//double new_channel = 2405.0 + (ASN%16)*5;
			double new_channel = 2405.0 + indch*5;
			trace() << "DIEGO new_channel=" << new_channel;
			...
			if(buffer_ret.size() > 0 && slots[slotCont]) {
				trace() << "DIEGO Linha 156 buffer of packets to be retransmitted";
				packet_ret *pr = buffer_ret.front();
				buffer_ret.pop_front();
				...
				if(pr->cont < this->numberOfAttempts) {
					...
					trace() << "DIEGO pr->cont < this->numberOfAttempts";
										
				}	
				
				trace() << "Retransmitting packet " << pr->packet->getSequenceNumber() << " on indch: " << indch << "  slotCont: " << slotCont;
			...
			//buffer of new packets
				...
				trace() << "DIEGO buffer of new packets buffer.size() > 0 && slots[slotCont], em que slotCont=" << slotCont << " slots[slotCont]=" << slots[slotCont];
				...
				if(p->ack && this->numberOfAttempts > 0) {
					...
					trace () << "Listening to ack";
					trace() << "DIEGO Linha 205 p->ack && this->numberOfAttempts > 0 Colocando pacote para retransmissão...";
				}
			...
			slotCont++;
			trace() << "DIEGO slotCont incrementado=" << slotCont;
			if(slotCont == superFrameSize){
				trace() << "DIEGO slotCont == superFrameSize, entao slotCont é zerado";
				slotCont = 0;
			}
			ASN++;
			trace() << "DIEGO ASN incrementado para " << ASN;
			break;
		}
	}
}
