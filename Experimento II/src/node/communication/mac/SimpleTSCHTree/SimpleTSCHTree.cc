...
void SimpleTSCHTree::timerFiredCallback(int index)
{
	...
	switch (index){
		...
		case SWITCH_CHANNEL: { 
			...
			if(slots[slotCont]) {
				id_rx = cluster_id;
				trace() << "DIEGO: cluster_id = " << cluster_id;
				//se tiver mais de um ch para receber, precisa saber qual e
				//precisa de um array para dizer os receptore para o no (colocar um codigo de definicao aqui)
			}
			...
			if(!isClusterHead && !isCoordinator) {
				...
				/*if(slot_ret[slotCont]) {
					//decidir se eu tenho prioridade
					//se tiver prioridade transmitir o pacote pela rota secundaria
					//calcular o canal a ser usado de acordo com o id do roteador da rota secundaria
					//colocar o endereco de destino como o endereco do roteador secundario
				} else*/
				...
				/*if(slots[slotCont] && gack[slotCont]) {
					//criar um pacote
					SimpleTSCHTreePacket *gackFrame = new SimpleTSCHTreePacket("TDMAMAC packet", MAC_LAYER_PACKET);
					macFrame->setKindTSCH(ACK_PACKET);
					gackFrame->setBitmap(gack_bitmap); //gack_bitmap eh um array de inteiro que vc usa para marcar os pacotes recebidos nos slots
					//transmite o pacote
				} else */
				if(buffer_ret.size() > 0 && slots[slotCont]) {
					packet_ret *pr = buffer_ret.front();
